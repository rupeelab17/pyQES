...
XML="$REPO_ROOT/examples/umep_workflow/qes/umep_larochelle.xml"
QES_DIR="$(dirname "$XML")"
OUT_DIR="$REPO_ROOT/examples/umep_workflow/output"
...
Environment:
  ...
  QES_BUILDINGS_SRC   Source buildings shapefile (default: buildings.shp)
...
prepare_buildings_clipped() {
  local dem="$1"
  local src="${QES_BUILDINGS_SRC:-$REPO_ROOT/examples/umep_workflow/buildings.shp}"
  local mask="${QES_BUILDINGS_MASK:-$REPO_ROOT/examples/umep_workflow/mask.shp}"
  local out="$REPO_ROOT/examples/umep_workflow/qes/buildings_clipped.shp"
...
SHP="$REPO_ROOT/examples/umep_workflow/qes/buildings_clipped.shp"
if [[ ! -f "$SHP" ]]; then
  echo "Error: clipped buildings shapefile not found at: $SHP" >&2
  echo "Set QES_BUILDINGS_SRC or run ogr2ogr -t_srs EPSG:2154 -clipsrc mask.shp qes/buildings_clipped.shp buildings.shp" >&2
  exit 1
fi
